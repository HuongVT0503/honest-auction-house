...
1.  **The "Rational Irrationality" of Revealing:**
    * If a user bids low and realizes they won't win, they have no economic incentive to pay gas (or effort) to "Reveal".
    * **Impact:** The auction might close with fewer valid bids than actual attempts, potentially altering the perceived market price.
2.  **Lazy Updates:**
    * Auctions rely on incoming API traffic to trigger state transitions. If no one requests `GET /auctions`, the auction technically stays "Open" in the database indefinitely.
3.  **Client-Side Compute:**
