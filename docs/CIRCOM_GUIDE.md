...
### Prover Artifact Deployment

For the frontend to generate proofs, the following files must be placed in `client/public/`. Because Vite serves the `public` folder at the root, the code fetches them from:

- `await snarkjs.groth16.fullProve(input, "/bid_check.wasm", "/bid_check_final.zkey")`
